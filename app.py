...
def search_venues():
    ...
    find_venues = Venue.query.filter(Venue.name.ilike(f'%{search_content}%') | Venue.city.ilike(
        f'%{search_content}%') | Venue.genres.ilike(f'%{search_content}%')).all()
    countNum = 0
    data = []
...
def search_artists():
    ...
    search_content = request.form.get('search_term', '')
    find_artists = Artist.query.filter(Artist.name.ilike(
        f'%{search_content}%') | Artist.genres.ilike(f'%{search_content}%')).all()
    countNum = 0
    data = []
